...
std::tm date_from_string(std::string s){
	std:: tm t = {};
...
std::string date_to_string(std::tm t){
	std::ostringstream sout;
	sout << std::put_time(&t, "%Y-%m-%d %H:%M:%S %Z (doy = %j)");
...
int _ymd2gday(int y, int m, int d){
	m = (m+9)%12;
	y = y - m/10;
...
double date_to_julian(std::tm time_struct){
